...
export class Enemy {
  ...
  update(dt: number) {
    ...
    if (this.perception <= 0) {
      ...
      if (this.visible) { this.lastSeen.copyFrom(this.player.position); this.memory = enemyConfig.searchDuration; this.state = 'COMBAT'; }
    ...
    } else if (this.state === 'SEARCH' || this.state === 'RETURN') {
      direction.copyFrom((this.state === 'SEARCH' ? this.lastSeen : this.spawn).subtract(this.body.position)); direction.y = 0; moving = direction.length() > 1.5;
    } else {
    ...
    if (direction.lengthSquared() > 0.01) {
      direction.normalize(); const target = Math.atan2(direction.x, direction.z);
